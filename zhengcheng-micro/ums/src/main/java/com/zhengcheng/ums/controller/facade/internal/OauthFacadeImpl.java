...
public class OauthFacadeImpl implements OauthFacade {
    ...
    public SaTokenInfo login(UserMobileLoginCommand userMobileLoginCommand, HttpServletRequest request) {
        ...
        // 前端对 code md5加密了
        if (CharSequenceUtil.isBlank(code) || !userMobileLoginCommand.getCode().equals(SecureUtil.md5(code))) {
    ...
    public void sendLoginSms(SendSmsCommand sendSmsCommand) throws UnsupportedEncodingException {
        ...
        if (CharSequenceUtil.isNotBlank(sendNumStr) && Objects.nonNull(sendNumStr) && Integer.parseInt(sendNumStr) >= 5) {
    ...
    private SaTokenInfo userLogin(User user, String loginIp) {
        ...
        String dayKey = CharSequenceUtil.format("{}{}", userActivityKeyPrefix, DateUtil.format(new Date(), PURE_DATE_FORMAT));
        // 每天按日期生成一个位图
        ...
        for (int offset = 0; offset < maxDayNum; offset++) {
            bytes[offset] = CharSequenceUtil.format("{}{}", userActivityKeyPrefix, DateUtil.format(DateUtil.offsetDay(new Date(), -offset), PURE_DATE_FORMAT)).getBytes();
        }
        String monthKey = CharSequenceUtil.format("{}{}", userActivityKeyPrefix, 30);
