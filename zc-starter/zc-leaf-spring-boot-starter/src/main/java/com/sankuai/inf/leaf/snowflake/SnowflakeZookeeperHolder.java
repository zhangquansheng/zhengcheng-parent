...
//    private static final String PATH_FOREVER = PREFIX_ZK_PATH + "/forever";//保存所有数据持久的节点
    ...
    private String getPrefixZkPath() {
        return "/snowflake/" + this.leafName;
    }

    private String getPropPath() {
        return System.getProperty("java.io.tmpdir") + java.io.File.separator + this.leafName + "/leafconf/{port}/workerID.properties";
    }

    private String getPathForever() {
        return this.getPrefixZkPath() + "/forever"; //保存所有数据持久的节点
    }
    ...
    public boolean init() {
        try {
            ...
            curator.start();
            Stat stat = curator.checkExists().forPath(this.getPathForever());
            if (stat == null) {
                ...
                java.util.Map<String, Integer> nodeMap = Maps.newHashMap();//ip:port->00001
                Map<String, String> realNode = Maps.newHashMap();//ip:port->(ipport-000001)
                //存在根节点,先检查是否有属于自己的根节点
                List<String> keys = curator.getChildren().forPath(this.getPathForever());
                for (String key : keys) {
                ...
                if (workerid != null) {
                    ...
                    zk_AddressNode = this.getPathForever() + "/" + realNode.get(listenAddress);
            ...
            try {
                ...
                properties.load(new FileInputStream(new File(this.getPropPath().replace("{port}", port + ""))));
    ...
    private String createNode(CuratorFramework curator) throws Exception {
        try {
            return curator.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT_SEQUENTIAL).forPath(this.getPathForever() + "/" + listenAddress + "-", buildData().getBytes());
        } catch (Exception e) {
    ...
    private void updateLocalWorkerID(int workerID) {
        File leafConfFile = new File(this.getPropPath().replace("{port}", port));
